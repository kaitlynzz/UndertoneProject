...
for i, path in enumerate(glob('UTKFace/*jpg')): #glob returns True or False, when True it returns the files within the directory of the identified file path
...
for i in tqdm(df.index): #iterate through the rows in the dataframe
    ...
    gender = path.split('\\')[1].split('_')[1]  #splits variable at the \ to create a list between UTKFace and 9_1_4..etc then it splits again at every _ at the FIRST index which then takes the first index once again
    ...
    df.loc[i, 'gender'] = gender #stores numerical value of gender into its proper ROW (picture #) under the gender column
    df.loc[i, 'age'] = age #stores numerical value of age into its proper ROW (picture #) under the age column
    df.loc[i, 'ethnicity'] = ethnicity #stores numerical value of ethnicity into its proper ROW (picture #) under the ethnicity column

df = df[df.ethnicity.apply(lambda x: x.isnumeric())]  #checks if the charracters within ethnicity dataframe are numerical
...
def get_face_coords_MTCNN(img):
    ...
    if results != []: #if it detects a face
        b = results[0]['box'] #because results is a list of one item, it takes the element at index 0 (the only element)
        #and uses the key 'box' to access  a list of coordinates
        x1 = int(b[0]) #takes the first value of the list and stores it
        x1 = (x1 if x1>0 else 0) #checks if this stored value is greater than 0 (still on the screen) if not, then set to 0
        #repeat
        y1 = int(b[1])
...
def resize_img(img, desired_height):
    try:
        ...
        img_height = int(height / resize_multiple) #the new height of the image found through dividing true height by resize
        img_width = int(height / resize_multiple) #the new width is the same as the height, found by the same method
        img = cv2.resize(img, None, fx=resize_multiple, fy=resize_multiple, interpolation=cv2.INTER_AREA) #actually resizes the image
        return img
    except:
        return img
...
def get_HSV_centroid(data_ori): #creates the clusters and centroids for our data
    clt = KMeans(n_clusters = 3, max_iter=5) #3 clusters therefore 3 centroids, max repeats while trying to stabilize are 5
